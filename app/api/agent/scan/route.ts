...
export async function POST(request: NextRequest) {
  ...
  for (const regulationName of regulationsToScan) {
    ...
    let conflictStatus: ConflictCheckStatus = "pending";
    ...
    if (otherRegulations.length > 0) {
      ...
      const checkRes = await callOpenAI<{ safe: boolean; conflicts: ConflictFlag[] }>({
      ...
      if (checkRes && !checkRes.safe && checkRes.conflicts?.length > 0) {
        ...
        if (healRes?.healed_actions) {
          ...
          conflictFlags = checkRes.conflicts.map((conflict) => ({
            ...conflict,
            resolution: healRes.healing_summary
          }));
